...
hip_roll = 0.0
hip_pitch = 0.0
filtered_back_roll = 0.0
filtered_back_pitch = 0.0
filtered_hip_roll = 0.0
filtered_hip_pitch = 0.0
filter_ready = False
...
def smooth_angle(previous, current):
    # Low-pass filter: keeps real movement visible while damping frame-to-frame jitter.
    return previous + POSE_SMOOTHING_ALPHA * (current - previous)


def apply_center_deadband(value):
    # Treat tiny near-neutral motion as sensor noise so the marker can settle.
    if abs(value) < CENTER_DEADBAND_DEG:
        return 0.0
...
def calibrate():
    ...
    global filtered_back_roll, filtered_back_pitch, filtered_hip_roll, filtered_hip_pitch
    global filter_ready
...
def compute_pose():
    ...
    global filtered_back_roll, filtered_back_pitch, filtered_hip_roll, filtered_hip_pitch, filter_ready
    ...
    if packet:
        ...
        if filter_ready:
            filtered_back_roll = smooth_angle(filtered_back_roll, back_roll)
            filtered_back_pitch = smooth_angle(filtered_back_pitch, back_pitch)
            filtered_hip_roll = smooth_angle(filtered_hip_roll, hip_roll)
            filtered_hip_pitch = smooth_angle(filtered_hip_pitch, hip_pitch)
        else:
            filtered_back_roll = back_roll
            filtered_back_pitch = back_pitch
            filtered_hip_roll = hip_roll
            filtered_hip_pitch = hip_pitch
            filter_ready = True

        last_packet_time = now

    # Relative posture is the back movement after removing hip movement.
    displayed_back_roll = apply_center_deadband(filtered_back_roll)
    displayed_back_pitch = apply_center_deadband(filtered_back_pitch)
    displayed_hip_roll = apply_center_deadband(filtered_hip_roll)
    displayed_hip_pitch = apply_center_deadband(filtered_hip_pitch)
    relative_roll = apply_center_deadband(displayed_back_roll - displayed_hip_roll)
    relative_pitch = apply_center_deadband(displayed_back_pitch - displayed_hip_pitch)
    ...
    return Pose(
        back_roll=displayed_back_roll,
        back_pitch=displayed_back_pitch,
        hip_roll=displayed_hip_roll,
        hip_pitch=displayed_hip_pitch,
...
class Dashboard(QMainWindow):
    ...
    def run_calibration(self):
        # Reuse startup calibration while the dashboard is already open.
        if self.calibrating:
            return
    ...
    def keyPressEvent(self, event):
        # Space recalibrates neutral posture without restarting the dashboard.
    ...
    def eventFilter(self, watched, event):
        # Catch Space before focused buttons can treat it as a click.
